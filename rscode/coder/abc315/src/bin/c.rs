...
fn main() {
    ...
    fs.sort_by_key(|x| std::usize::MAX - x.1);
    let mut result1: usize = 0;
    let mut result: usize = 0;
    if fs[0].0 != fs[1].0 {
        println!("{}", fs[0].1 + fs[1].1);
        return;
    }
    for i in 0..n {
        if v[fs[i].0].len() < 3 {
            v[fs[i].0].push(fs[i].1);
        }
    }
    for i in 0..n {
        if v[i].len() >= 2 {
            result1 = max(result1, v[i][0] + v[i][1] / 2);
        }
    }
    for i in 0..n {
        if fs[0].0 != fs[i].0 {
            result = fs[0].1 + fs[i].1;
            break;
        }
    }
    println!("{}", max(result, result1));
}
